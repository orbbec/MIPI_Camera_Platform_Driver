...
export L4T_VERSION="jetson_39.2"
export KERNEL_DIR="kernel/kernel-noble"
...
if [[ $JETPACK_VERSION == "7.2" ]]; then
    L4T_VERSION="jetson_39.2"
    KERNEL_DIR="kernel/kernel-noble"
elif [[ $JETPACK_VERSION == "7.1" ]]; then
    L4T_VERSION="jetson_38.4"
    KERNEL_DIR="kernel/kernel-noble"
elif [[ $JETPACK_VERSION == "7.0" ]]; then
    L4T_VERSION="jetson_38.2"
    KERNEL_DIR="kernel/kernel-noble"
    ...
    echo "Wrong JetPack version ($JETPACK_VERSION)! Only 7.2, 7.1, 7.0, 6.2, 6.1, 6.0, 5.1.2, 5.0.2 and 4.6.1 supported."
    exit 1
fi

if [[ "$JETPACK_VERSION" == "7.2" ]] || [[ "$JETPACK_VERSION" == "7.0" ]] || [[ "$JETPACK_VERSION" == "7.1" ]]; then
    export CROSS_COMPILE=$DEVDIR/l4t-gcc/$JETPACK_VERSION/x-tools/aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-
...
if [[ "$JETPACK_VERSION" == "7.2" ]] || [[ "$JETPACK_VERSION" == "7.0" ]] || [[ "$JETPACK_VERSION" == "7.1" ]]; then
    cd $SRCS
    export KERNEL_HEADERS=$SRCS/kernel/kernel-noble
    export kernel_name=noble

    # If building a real-time kernel
    ...
    cp $SRCS/build/nvidia-public/devicetree/generic-dtbs/* $TEGRA_KERNEL_OUT/rootfs/boot/dtb/
    ...
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3768-0000+p3767-0000-nv.dtb $TEGRA_KERNEL_OUT/rootfs/boot/dtb/
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3768-0000+p3767-0001-nv.dtb $TEGRA_KERNEL_OUT/rootfs/boot/dtb/
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3767-camera-p3768-g300-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3767-camera-p3768-g300-nomtd-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3737-camera-g300-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    ...
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3737-camera-g300-fg96-obcam-thin-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    cp $SRCS/nvidia-oot/device-tree/platform/generic-dts/dtbs/tegra234-p3737-camera-g300-leopard-obcam-thin-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    export INSTALL_MOD_PATH=$TEGRA_KERNEL_OUT/rootfs/
    make ARCH=arm64 install -C kernel
    make ARCH=arm64 modules_install
elif [[ "$JETPACK_VERSION" == "6.1" ]] || [[ "$JETPACK_VERSION" == "6.2" ]]; then
    ...
    make ARCH=arm64 dtbs
    mkdir -p $TEGRA_KERNEL_OUT/rootfs/boot/dtb
    cp $SRCS/kernel-devicetree/generic-dts/dtbs/tegra234-p3737-camera-g300-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    cp $SRCS/kernel-devicetree/generic-dts/dtbs/tegra234-p3737-camera-g300-nomtd-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    ...
    cp $SRCS/kernel-devicetree/generic-dts/dtbs/tegra234-p3767-camera-p3768-g300-nomtd-fg96-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    ...
    cp $SRCS/kernel-devicetree/generic-dts/dtbs/tegra234-p3737-camera-g300-fg96-obcam-thin-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    cp $SRCS/kernel-devicetree/generic-dts/dtbs/tegra234-p3737-camera-g300-leopard-obcam-thin-overlay.dtbo $TEGRA_KERNEL_OUT/rootfs/boot/
    export INSTALL_MOD_PATH=$TEGRA_KERNEL_OUT/rootfs/
    make ARCH=arm64 install -C kernel
    make ARCH=arm64 modules_install
else
