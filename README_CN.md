...
## GMSL接入操作步骤
  ...
  ### 软件驱动安装
   ...
   - Orbbec G300 系列 GMSL 相机驱动快速使用指南
     - 完整编译、安装、设备树适配和验证流程请参考[Orbbec G300 系列 GMSL 相机驱动快速使用指南](./doc/Orbbec_G300_Series_GMSL_Camera_Driver_Quick_Start_Guide_CN.md)
...
## GMSL使用限制说明
  GMSL相机要接入不同的9296/92716等不同的解串芯片；奥比GMSL相机同时支持深度、彩色、IR、IMU等多路流，在使用方面有一些限制；
   ...
   - G335Lg metadata 默认是单独的Video节点，V1.5.70固件版本后支持设置 metadada 放在视频帧数据的第一行数据里（设备树中设置embedded_metadata_height = "0";使用我们编译好的驱动则使用带nomtd 标志的设备树）。G345Lg 默认是把metadada 放在视频帧数据的第一行里，目前暂不支持设置 。
   ...
   - 接在同一Max9296、Max96712 LinkA/B 、Max96712 LinkC/D 上的两个相机使用有以下限制
     -  驱动版本v1.2.02前限制为一个相机的RGB 跟另一个相机的右IR 不能同时开流,驱动版本v12.02后修改限制为一个相机的RGB 跟另一个相机的左IR 不能同时开流
     -  驱动版本v1.2.02前限制为一个相机的DEPTH 跟另一个相机的左IR 不能同时开流，驱动版本v12.02后修改限制为一个相机的DEPTH 跟另一个相机的右IR 不能同时开流
