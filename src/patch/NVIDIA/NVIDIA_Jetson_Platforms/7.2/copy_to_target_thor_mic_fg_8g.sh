...
sudo cp tegra264-p4071-camera-g300-mic-fg-8g-overlay.dtbo /boot/tegra264-camera-g300-overlay.dtbo
...
if [ ! -f $nvhost_nvcsi_dir/nvhost-nvcsi.ko.orig ];then
    echo "bakckup nvhost-nvcsi.ko"
    sudo cp $nvhost_nvcsi_dir/nvhost-nvcsi.ko $nvhost_nvcsi_dir/nvhost-nvcsi.ko.orig
fi
sudo cp nvhost-nvcsi.ko $nvhost_nvcsi_dir
sudo sync
...
echo "Configure the pinmux of the Jetson SOC for the Jetson Orbbec Camera G300."
sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 1="Jetson Orbbec Camera G300"
